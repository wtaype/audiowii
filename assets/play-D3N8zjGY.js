import{j as e}from"./vendor-gzd0YkcT.js";import{N as c}from"./main-DfBTo_Yi.js";import"./main-BcrffMmT.js";import"./firebase-BVkF6YOI.js";const G=()=>`
  <div class="wi_play">
...
`,a={files:[],idx:0,pasteN:1,looping:!1,speed:1,vol:70,searchQ:"",searching:!1,actx:null,analyser:null,dataArr:null,srcNode:null,rafId:null},L=["audio/mp3","audio/mpeg","audio/wav","audio/m4a","audio/ogg","audio/aac","audio/flac","audio/webm"],h=[.5,.75,1,1.25,1.5,2,3],m=[0,25,50,70,100],E=l=>l.type.startsWith("audio/")||L.includes(l.type),C=l=>l?`${(l/1024**Math.floor(Math.log(l)/Math.log(1024))).toFixed(1)} ${["B","KB","MB"][Math.floor(Math.log(l)/Math.log(1024))]}`:"0 B",f=(l=0)=>`${String(Math.floor(l/60)).padStart(2,"0")}:${String(Math.floor(l%60)).padStart(2,"0")}`,r=()=>e("#play_audio")[0],S=()=>a.searchQ?a.files.filter(l=>l.name.toLowerCase().includes(a.searchQ)):a.files,M=()=>{a.searching=!a.searching,a.searching?(e("#play_header").addClass("dpn"),e("#play_search_bar").removeClass("dpn"),setTimeout(()=>e("#play_search_input").focus(),100)):g()},g=()=>{a.searching=!1,e("#play_search_bar").addClass("dpn"),e("#play_header").removeClass("dpn"),e("#play_search_input").val(""),a.searchQ="",d()},R=l=>{a.searchQ=l.toLowerCase().trim(),d();const t=S();e("#play_search_results").text(`${t.length} de ${a.files.length}`)},x=(l,t=!1)=>{const s=Array.from(l).filter(i=>E(i)?!0:(c(`${i.name} no es audio`,"error",2e3),!1));if(!s.length)return;const n=a.files.length;s.forEach(i=>{const p=URL.createObjectURL(i),_={id:Date.now()+Math.random(),name:t?`Audio_${a.pasteN++}.mp3`:i.name,format:i.type,size:i.size,url:p,isPasted:t,duration:0},o=new Audio;o.preload="metadata",o.onloadedmetadata=()=>{_.duration=o.duration,d(),o.src=""},o.onerror=()=>o.src="",o.src=p,a.files.push(_)}),c(t?"Audio pegado":`${s.length} audio(s) agregado(s)`,"success",1500),d(),(t||n===0)&&y(t?a.files.length-1:0)},F=l=>{const t=l.originalEvent?.clipboardData?.items;if(t){for(let s=0;s<t.length;s++)if(t[s].type.startsWith("audio/")){const n=t[s].getAsFile();if(n){x([n],!0),e("#play_zone").addClass("paste_flash"),setTimeout(()=>e("#play_zone").removeClass("paste_flash"),300);return}}t.length&&c("No se detectó audio en portapapeles","warning",2e3)}},d=()=>{const l=e("#play_gallery"),t=S();if(e("#play_count").text(a.files.length),!a.files.length){l.html('<div class="play_gallery_empty"><i class="fas fa-folder-open"></i><p>Sin audios</p></div>');return}if(!t.length&&a.searchQ){l.html('<div class="play_gallery_empty"><i class="fas fa-search"></i><p>Sin resultados</p></div>');return}l.html(t.map(s=>{const n=a.files.indexOf(s),i=a.searchQ?s.name.replace(new RegExp(`(${a.searchQ})`,"gi"),"<mark>$1</mark>"):s.name;return`
  ...
  `),setTimeout(()=>e("#play_vol_fill").css("width",`${a.vol}%`),10)},k=()=>{const l=r();l?.src&&(l.paused?l.play():l.pause())},N=()=>{a.looping=!a.looping,e("#play_btn_loop").toggleClass("active",a.looping),c(`Loop ${a.looping?"ON":"OFF"}`,"info",1200)},P=()=>{const l=(h.indexOf(a.speed)+1)%h.length;a.speed=h[l],r().playbackRate=a.speed,e("#play_btn_speed").text(`${a.speed}x`),c(`Velocidad: ${a.speed}x`,"info",1200)},O=()=>{const l=(m.indexOf(a.vol)+1)%m.length;a.vol=m[l],z(),c(`Volumen: ${a.vol}%`,"info",1200)},z=()=>{const l=r();l.volume=a.vol/100,e("#play_vol_fill").css("width",`${a.vol}%`);const t=a.vol===0?"fa-volume-mute":a.vol<50?"fa-volume-down":"fa-volume-up";e("#play_btn_vol i").attr("class",`fas ${t}`)},Q=()=>{if(!a.files.length)return;const l=a.files[a.idx],t=document.createElement("a");t.href=l.url,t.download=l.name,t.click(),c(`${l.name} descargado`,"success",1500)},b=()=>y(a.idx<a.files.length-1?a.idx+1:0),$=()=>y(a.idx>0?a.idx-1:a.files.length-1),w=()=>{const l=r();l?.src&&(l.pause(),l.currentTime=0),a.rafId&&(cancelAnimationFrame(a.rafId),a.rafId=null)},A=()=>{const l=e('<input type="file" accept="audio/*" multiple style="display:none">');l.on("change",function(){this.files.length&&x(this.files),l.remove()}),e("body").append(l),l.click()},V=()=>{a.files.length&&(a.files.forEach(l=>URL.revokeObjectURL(l.url)),a.files=[],a.idx=0,a.pasteN=1,w(),e(".play_placeholder").removeClass("dpn"),e(".play_player, #play_info").addClass("dpn"),d(),c("Todo limpiado","success",1500))},B=l=>{l<0||l>=a.files.length||(URL.revokeObjectURL(a.files[l].url),a.files.splice(l,1),a.files.length?l===a.idx?y(Math.min(l,a.files.length-1)):l<a.idx&&a.idx--:(w(),e(".play_placeholder").removeClass("dpn"),e(".play_player, #play_info").addClass("dpn")),d())},D=()=>{e("#play_zone").on("dblclick",A).on("dragover",l=>{l.preventDefault(),e("#play_zone").addClass("dragover")}).on("dragleave",()=>e("#play_zone").removeClass("dragover")).on("drop",l=>{l.preventDefault(),e("#play_zone").removeClass("dragover");const t=l.originalEvent.dataTransfer?.files;t?.length&&x(t)}),e(document).on("paste.play",F),e(".play_btn_add").on("click",A),e(".play_btn_clear").on("click",V),e(".play_btn_search").on("click",M),e("#play_btn_close_search").on("click",g),e("#play_search_input").on("input",function(){R(e(this).val())}),e(document).on("click.play","#play_btn_play",k),e(document).on("click.play","#play_btn_prev",$),e(document).on("click.play","#play_btn_next",b),e(document).on("click.play","#play_btn_loop",N),e(document).on("click.play","#play_btn_speed",P),e(document).on("click.play","#play_btn_vol",O),e(document).on("click.play","#play_btn_download",Q),e(document).on("click.play","#play_timeline_click",function(l){const t=r();t?.duration&&(t.currentTime=l.offsetX/e(this).width()*t.duration)}),e(document).on("click.play","#play_vol_click",function(l){a.vol=Math.round(l.offsetX/e(this).width()*100),z()}),e(document).on("click.play",".play_gallery_item",function(){y(parseInt(e(this).data("i")))}),e(document).on("click.play",".play_btn_del",function(l){l.stopPropagation(),B(parseInt(e(this).data("i")))}),e(document).on("keydown.play",l=>{if(a.searching){l.key==="Escape"&&g();return}a.files.length&&([" ","ArrowLeft","ArrowRight"].includes(l.key)&&l.preventDefault(),l.key===" "&&k(),l.key==="ArrowLeft"&&$(),l.key==="ArrowRight"&&b())})},X=()=>{D(),console.log("✅ Play cargado")};export{X as init,G as render};
